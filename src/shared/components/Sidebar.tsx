...
export const Sidebar = ({ collapsed, setCollapsed }: SidebarProps) => {
  ...
  return (
    <aside
      className={`
        bg-surface text-text flex flex-col transition-all duration-300 ease-in-out overflow-hidden
...
const SidebarItem = ({ item, collapsed, isOpen, onClick }: ItemProps) => {
  ...
  const className = `
    cursor-pointer w-full flex items-center p-2 rounded transition-colors group text-left
    ${
      isActive
...
const SidebarSubItem = ({ label, path }: { label: string; path: string }) => {
  ...
  return (
    <Link
      to={path}
      className={`
        block p-2 text-xs rounded cursor-pointer transition-colors
        ${
          isActive
            ? "text-text font-bold bg-background/50"
